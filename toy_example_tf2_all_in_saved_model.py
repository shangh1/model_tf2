...
from toy_tf_util import MecKaminoFeature, splitFL
...
def main_fun(mode):
    ...
    def build_tf_api():
        ...
        inputs_0 = Input(shape=(1, ), dtype='string')
        inputs_0 = [inputs_0]
        batch_ids, labels, input_ids = MecKaminoFeature(inputs_0, 'vocab.txt', sequence_length, num_classes)
        inputs = {'ids':batch_ids[0], 'input_feature':input_ids[0]}
    ...
    def get_data(_input):
        ...
        datasets_unbatched = get_unbatched_data(input_files, mode, epochs)
        data_batch = datasets_unbatched.batch(GLOBAL_BATCH_SIZE)
        return data_batch
    ...
    if mode == 'train':
        ...
        train_data = get_data(input_file)
        validation_data = get_data(val_path_file)
        histCallback = multi_worker_model.fit(x=train_data, validation_data=validation_data, epochs=epochs, steps_per_epoch=steps_per_epoch, callbacks=callbacks)
        for key in histCallback.history:
            print(key)
        results = multi_worker_model.evaluate(validation_data, verbose=2)
        ...
        test_data = get_data(test_path_file)
